 ...
 <div class="admin_info_style">
   <div class="admin_modify_style" id="Personal">
     <div class="type_title">管理员信息 </div>
      <div class="xinxi">
      <form name="form1" action="<%=path %>/Change_infoServlet?action=change_info" method="post">
        <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">用户名：</label>
          <div class="col-sm-9"><input type="text" name="txt_name" id="website-title" value="<%=request.getAttribute("txt_name") %> " class="col-xs-7 text_info" disabled="disabled">
          ...
          <div class="col-sm-9">
            <span class="sex"><%=request.getAttribute("sex") %> </span>
            <div class="add_sex">
          ...
          <div class="col-sm-9"><input type="text" name="age" id="website-title" value="<%=request.getAttribute("age") %> " class="col-xs-7 text_info" disabled="disabled"></div>
          </div>
          <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">移动电话： </label>
          <div class="col-sm-9"><input type="text" name="tel" id="website-title" value="<%=request.getAttribute("tel") %> " class="col-xs-7 text_info" disabled="disabled"></div>
          </div>
          <div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">电子邮箱： </label>
          <div class="col-sm-9"><input type="text" name="email" id="website-title" value="<%=request.getAttribute("email") %> " class="col-xs-7 text_info" disabled="disabled"></div>
          </div>
          <!--<div class="form-group"><label class="col-sm-3 control-label no-padding-right" for="form-field-1">QQ： </label>
          <div class="col-sm-9"><input type="text" name="QQ" id="website-title" value="XXXXXXXXXXX" class="col-xs-7 text_info" disabled="disabled"> </div>
          </div>-->
 ...
 //初始化宽度、高度    
  ...
  function change_Password(){
	...
	yes:function(index, layero){
		   if ($("#password").val()!=<%=request.getAttribute("pwd")%>){
			  layer.alert('原密码不正确!',{
              title: '提示框',				
				icon:0,
		    ...
		    if(!$("#c_mew_pas").val || $("#c_mew_pas").val() != $("#Nes_pas").val() )
        {
            layer.alert('密码不一致!',{
			 ...
			 location = "<%=request.getContextPath()%>/Change_infoServlet?action=change_pwd&pwd="+$("#Nes_pas").val(); 
			  layer.alert('修改成功！',{
               title: '提示框',				
			icon:1,		
			  }); 
