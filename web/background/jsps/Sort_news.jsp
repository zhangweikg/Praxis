 ...
 <div class="sort_style">
 	<div class="search_style">
      ...
      <ul class="search_content clearfix">
       <li><label class="l_f">标题关键字</label><input name="title" type="text"  class="text_add" placeholder="输入标题关键字"  style=" width:250px"/></li>
       <li><label class="l_f">内容关键字</label><input name="content" type="text"  class="text_add" placeholder="输入内容关键字"  style=" width:250px"/></li>
       <li style="width:90px;"><button type="button" class="btn_search"><i class="icon-search"></i>查询</button></li>
      </ul>
    </div>
    <div class="border clearfix">
       <span class="l_f">
        <a href="javascript:ovid()" id="sort_add" class="btn btn-warning"><i class="fa fa-plus"></i> 添加新闻</a>
   ...
   <% List<News> list = (ArrayList)request.getAttribute("list"); %>
	...
	<%for (int i =0; i<list.size(); i++) {%>
      <tr>
       ...
       <td><%=list.get(i).getNews_id() %></td>
       <td><a href="<%=path%>/NewsDetailServlet?id=<%=list.get(i).getNews_id() %>"><%=list.get(i).getNews_title() %></a></td>
       <td><img alt="" src="<%=path %>/foreground/img/<%=list.get(i).getNews_picture_path() %>" width="250px"  /></td>
       <td><%=list.get(i).getNews_publish_time() %></td>
      <td  align="center">
        <a href="ads_update.jsp" id="news_update" class="btn btn-warning">修改</a>
...
<div class="sort_style_add margin" id="sort_style_add" style="display:none">
  <div class="">
     <ul>
      <li><label class="label_name">标题</label><div class="col-sm-9"><input name="新闻标题" type="text" id="form-field-1" placeholder="" class="col-xs-10 col-sm-5"></div></li>
      <li><label class="label_name">内容</label><div class="col-sm-9"><textarea name="新闻内容" class="form-control" id="form-field-8" placeholder="" onkeyup="checkLength(this);"></textarea><span class="wordage">剩余字数：<span id="sy" style="color:Red;">500</span>字</span></div></li>
     </li>
     </ul>
  </div>
</div>
...
$('.Order_form ,.ads_link').on('click', function(){
	...
	//parent.$('.Current_page').jsp("<a href='javascript:void(0)' name="+herf+">" + cnames + "</a>");
